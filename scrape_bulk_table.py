...
for cik in ciks[0:10]:
    ...
    for year in years:
        try:
            year_data = scrape_table.get_sheet(
                cik, "10-K", f"{year-1}0101", f"{year}0101")
...
for cik in ciks[0:10]:
    ...
    for year in years:
        for i in range(len(months)-1):
            try:
                quarter_data = scrape_table.get_sheet(
                    cik, "10-Q", f"{year-1}" + months[i] + "01", f"{year-1}" + months[i+1] + "01")
